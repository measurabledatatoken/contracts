...
/**
 * @title The MDTokenLockupTest contract
 * @dev Lock up MDT and receive bonus
 * @author MDT Team
 */
contract MDTokenLockupTest is ERC677Receiver, Ownable {
    ...
    struct Lockup {
        ...
        uint256 fakeEndTime; // for testing only
    }
    ...
    modifier onlyAfterLockEvent {
        require(hasEnded() || isTesting);
        _;
    }

    /**
    * @dev MDTokenLockupTest contract constructor.
    * @param _tokenAddress address The MDT contract address.
    * @param _endTime uint256 The end time of the lockup event.
    */
    function MDTokenLockupTest(address _tokenAddress, uint256 _endTime, bool _isTesting) public {
    ...
    function _lockupTokens(address _sender, uint256 _amount, uint8 _lockupPeriod, Lockup storage lockup, bool lockPrivateSale) private {
        ...
        if (isTesting) {
            uint256 base = 5 minutes;
            lockup.fakeEndTime = now.add(base.mul(_lockupPeriod + 1));
        }
    ...
    function canWithdrawTokens(bool isPrivateSale) public view returns (bool) {
        Lockup storage lockup = getTokenLockup(msg.sender, isPrivateSale);
        return lockup.value > 0 && (now >= lockup.endTime || (isTesting && now >= lockup.fakeEndTime)) && lockup.withdrawn == false;
    }
     ...
     */
    function withdrawTokens(bool isPrivateSale) public /*onlyAfterLockEvent*/ returns (uint256) {
